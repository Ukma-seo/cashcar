...
String.prototype.toUnicode = function() {
  var result = "";
  if (CONSTANTS.english.test(this)) return this.toString();
  if (CONSTANTS.isnumber.test(this)) return parseInt(this);
...
function init() {
  for (let i = 0; i < 5; i++) {
    var currKey = Object.keys(CONSTANTS.data)[i];
    insertOptions(currKey, CONSTANTS.data[currKey]);
  }
  fill_model();
  fill_prod_date();
  add_event_listeners();
...
function fill_model() {
  ...
  insertOptions(
    "model",
    CONSTANTS.data["brand_model_combinations"][chosen_brand]
  );
}

function fill_prod_date() {
  var year_options_array = [];
  for (let i = 1980; i < 2020; i++) {
    year_options_array.push(i);
  }
  insertOptions("prod_date", year_options_array);
}

function handle_select_model(event) {
  var val = event.target.value.toString().replace("_", " ");
  insertOptions("model", CONSTANTS.data["brand_model_combinations"][val]);
}
...
function handle_autocomplete() {
  ...
  const fitting_pages = search_for_words(input, CONSTANTS.pages);
...
async function send() {
  var body = {};
  CONSTANTS.features.forEach((feature, index) => {
    body[feature] =
      document.getElementById(feature).value.toUnicode() ||
      CONSTANTS.default_features[index];
  });
  body["with_auction"] = 0;
